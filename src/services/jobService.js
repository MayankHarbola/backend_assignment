...
const payJob = async (req) => {
  ...
  if (balance < jobPrice) return {success: false, message: "user have Insufficient balance"};
  if (type == 'contractor') return {success: false, message: "Jobs payment can be done by contractor only"};
